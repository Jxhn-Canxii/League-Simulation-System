<template>
    <div>
        <div class="w-full mb-2 flex overflow-x-auto border-b-2" v-if="season_info.conferences && season_info.seasons">
            <ul class="flex flex-wrap">
                ...
                >
                    ...
                    <span
                        ...
                        >{{ conference.name }}   {{ conference.champions_count > 0 ? '( '+conference.champions_count+' )' : '' }}</span
                    >
                    <!-- Warning Badge Notification Counter -->
                </li>
            </ul>
        </div>
        <div class="flex justify-end mb-2" v-if="season_schedules && !season_schedules.is_simulated">
            <button
                @click="simulateConference()"
                :disabled="isHide"
    ...
    >
        ...
        <div
            class="md:col-span-3 sm:col-span-1 overflow-y-auto"
        >
            <h2
                    ...
                    >
                        ...
                        >
                            <button
                                type="button"
                                class="uppercase"
                                :title="'Conference champions:'+team.conference_1_rank+' Playoff Appearance:'+team.playoff_appearances"
                                @click.prevent="isTeamModalOpen =  team.team_id"
                            >
                                <b>{{ team.team_name }}</b>
                            </button>
                        </td>
            ...
            <ul class="mt-4 uppercase" v-if="season_info.seasons">
                <li>
                    <i class="fas fa-trophy"></i>Finals
                    Champion:
                    {{
                ...
                <li>
                    <i class="fas fa-medal"></i>Finals Runner
                    Up:
                    {{
        ...
        <div
            class="md:col-span-4 sm:col-span-1 overflow-y-auto"
        >
            <h2
                class="text-lg font-semibold text-gray-800 mb-2"
            >
                Schedule and Results ({{ season_schedules?.schedules.length }})
            </h2>
            ...
            >
                ...
                >
                    <div class="px-4 py-5 sm:px-6">
                        ...
                        <code
                            class="mt-1 max-w-2xl text-xs text-gray-300"
                        >
                            #R{{ game.round }}-{{
                                game.game_id
                            }}
                            ...
                            >
                                ...
                                >
                                    <button
                                        ...
                                        "
                                        class="text-indigo-600 bg-orange-300 p-2 rounded-full text-md font-bold text-nowrap hover:text-indigo-900"
                                    >
                                        Simulate Game
                                    </button>
                                    <button
                                        ...
                                        v-if="
                                            ...
                                            2
                                        "
                                        class="text-indigo-600 bg-orange-200 p-2 rounded-full text-md font-bold text-nowrap hover:text-indigo-900"
                                    >
                                        Simulate All Round
                                    </button>
                                </div>
                                ...
                                >
                                    <p class="text-red-500 animate-pulse text-xs text-nowrap">Getting results</p>
                                </div>
                            </template>
                            <template v-else>
                                <div
...
const simulateGame = async (id,type,index,round) => {
    try {
        ...
        const results = response.data.schedule;
            await fetchSeasonStandings(props.season_id,change_key.value);
            season_schedules.schedules[index] = results;
        isHide.value = false;
...
const simulateAll = async (id) => {
    try {
        isHide.value = true;
        const response = await axios.post(route("game.simulate.all"), {
            season_id: props.season_id, // Assuming the parameter name should be schedule_id
        });
        // await localStorage.setItem('season-key',generateRandomKey());
        await fetchConferenceStandings(id);
...
const getStreakClass = (streakStatus) => {
        if (streakStatus == null) {
        return 'text-gray-600'; // Class for win streak
      }
      if (streakStatus.includes('W')) {
        return 'text-green-600'; // Class for win streak
      } else if (streakStatus.includes('L')) {
        return 'text-red-600'; // Class for lose streak
      } else {
        return ''; // Default class if no streak status
      }
    }
